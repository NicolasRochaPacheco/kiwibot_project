...
9. **[+5%/5.0]**: use `Valgrind` to analyses the `speed_controller` executable. [How to use it](./valgrind_usage.md). Why use `Valgrind`?
  * Created a [bash script](../robotics/configs/valgrind_sc.sh) that analyzes the `speed controller` executable. The executable requires a dynamic library (libsoft__speed.a) thus it is added to `LD_LIBRARY_PATH`. Valgrind is executed and the output is stored in a text file. Valgrind is a powerful tool to evaluate the memory performance of the executables. This is extremely useful in context in which the memory is a limited resource (such as an embedded system). In terms of security, valgrind also helps in preventing memory leaks that could be exploited by malware.
